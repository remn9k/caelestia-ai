...
CustomMouseArea {
    ...
    onContainsMouseChanged: {
        if (!containsMouse) {
            const aiHovered = popouts.currentName === "ai" && popouts.current && popouts.current.hovered === true;
            ...
            if (!aiHovered && (!popouts.currentName.startsWith("traymenu") || ((popouts.current as StackView)?.depth ?? 0) <= 1)) {
    ...
    onPositionChanged: event => {
        ...
        if (x < bar.implicitWidth) {
            bar.checkPopout(y);
        } else if (!(popouts.currentName === "ai" && popouts.current && popouts.current.hovered === true)
                   && (!popouts.currentName.startsWith("traymenu") || ((popouts.current as StackView)?.depth ?? 0) <= 1)
                   && !inLeftPanel(panels.popouts, x, y)) {
            popouts.hasCurrent = false;
            bar.closeTray();
        }
    }
